...
if not all([API_ID, API_HASH, CHANNEL_ID, WEBHOOK_URL]):
    raise ValueError("Configure todas as variáveis: API_ID, API_HASH, CHANNEL_ID, WEBHOOK_URL")

# Cliente Telegram (sessão em /data)
...
<html><head><meta charset="UTF-8"><title>Telegram Listener</title>
<style>
...
.success{{color:#28a745}}.error{{color:#dc3545}}a{{color:#0088cc;text-decoration:none}}
...
async def home():
    if await client.is_user_authorized():
        return page(f'<h2 class="success">✅ Conectado</h2><p>Monitorando canal {CHANNEL_ID}</p><p><a href="/health">Status</a></p>')
    return page('''<h2>🔐 Login</h2><form action="/send_code" method="post">
<input type="tel" name="phone" placeholder="+5511999999999" required>
<button>Enviar Código</button></form>''')
...
async def send_code(phone: str = Form(...)):
    ...
    try:
        ...
        return page(f'''<h2>📱 Código Enviado</h2><form action="/verify_code" method="post">
<input type="hidden" name="phone" value="{phone}">
<input type="text" name="code" placeholder="12345" required autofocus>
<button>Verificar</button></form>''')
    except PhoneNumberInvalidError:
        return page('<h3 class="error">❌ Número inválido</h3><p><a href="/">Voltar</a></p>')
    except SendCodeUnavailableError:
        return page(f'''<h3 class="error">⚠️ Muitas tentativas</h3><p>Aguarde. Já tem código?</p>
<form action="/verify_code" method="post"><input type="hidden" name="phone" value="{phone}">
<input type="text" name="code" required><button>Verificar</button></form>''')
...
async def verify_code(phone: str = Form(...), code: str = Form(...)):
    try:
        ...
        await start_listener()
        return page('<h2 class="success">✅ Login OK!</h2><p><a href="/">Status</a></p>')
    except PhoneCodeInvalidError:
        return page(f'''<h3 class="error">❌ Código inválido</h3>
<form action="/verify_code" method="post"><input type="hidden" name="phone" value="{phone}">
<input type="text" name="code" required autofocus><button>Tentar</button></form>''')
    except SessionPasswordNeededError:
        return page(f'''<h2>🔒 Senha 2FA</h2><form action="/verify_password" method="post">
<input type="hidden" name="phone" value="{phone}">
<input type="password" name="password" placeholder="Senha" required autofocus>
<button>Confirmar</button></form>''')
    except Exception as e:
        return page(f'<h3 class="error">❌ {e}</h3><a href="/">Voltar</a>')
